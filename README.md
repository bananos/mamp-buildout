...
Run supervisor
---------------
Once compilation is done, run `supervisor <http://supervisord.org/>`_, a client/server system that allows its users
to monitor and control a number of processes on UNIX-like operating systems.

```bash

bananos@amber: ~/Projects/github/mamp-buildout (master) $ bin/supervisord
...
bananos@amber: ~/Projects/github/mamp-buildout (master) $ bin/supervisorctl status
...
bananos@amber: ~/Projects/github/mamp-buildout (master) $ ls -l
 README.md
 bin
 bootstrap.py
 buildout-base.cfg
...
not int ``etc/`` which is recreated each time buildout is run.
...
bananos@amber: ~/Projects/github/mamp-buildout (master) $ bin/mysql --protocol=TCP -u root --port=3305 --host=localhost
...
```bash
bananos@amber: ~/Projects/github/mamp-buildout (master) $ ls -l var/log/
total 32
...
bananos@amber: ~/Projects/github/mamp-buildout (master) $
...
bananos@amber: ~/Projects/github/mamp-buildout (master) $ bin/supervisorctl tail httpd
...
Why not `MAMP <http://www.mamp.info/>`_?
-----------------------------------------
